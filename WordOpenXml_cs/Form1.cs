...
namespace WordOpenXml_cs
{
    public partial class Form1 : Form
    {
        ...
        private void cmdCreateNewDocumentWithTableFromArray_Click(object sender, EventArgs e)
        {
            ...
            if (ops.CreateDocumentWithTableFromArray(fileName))
            {
                /*
                 * Upon success on creating our document with a table of names,
                 * open this document and change the last name on row two from
                 * Bill Smith to Bill Jones
                 */
                ops.ChangeLastNameForSecondRow(fileName, "Jones");
                ((Button)sender).ImageKey = "Success";
            }
            else
            {
